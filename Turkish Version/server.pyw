...
def addmsg(message):
    ...
    messages.see(tk.END)
def closing():
    addmsg(f"[{clock}] Sunucu kapatıldı.")
    open("logs.txt", "a", encoding="utf-8").write(f"[{clock}] Sunucu kapatıldı.\n")
    os._exit(0)
        ...
        with open("banned.txt", "r", encoding="utf-8") as file:
            for i in file:
                if(i.strip() == client_address[0]):
                    addmsg(f"[{clock}] {connected_address[0]} banlandı.")
                    open("logs.txt", "a", encoding="utf-8").write(f"[{clock}] {connected_address[0]} banlandı.\n")
                    client_socket.close()
                    return
...
def listen(client_socket, client_address, username):
    try:
        ...
        addmsg(f"[{clock}] [{client_address[0]}] {username} bağlandı.")
        open("logs.txt", "a", encoding="utf-8").write(f"[{clock}] [{client_address[0]}] {username} bağlandı.\n")
        broadcast(f"[{time.strftime("%H:%M")}] {username} sohbete katıldı.", client_socket, client_address)
        ...
        addmsg(f"[{clock}] [{client_address[0]}] {username} sohbetten ayrıldı.")
        open("logs.txt", "a", encoding="utf-8").write(f"[{clock}] [{client_address[0]}] {username} sohbetten ayrıldı.\n")
        remove(client_socket, client_address)
...
def control(client_socket, client_address, username):
    with open("banned.txt", "r", encoding="utf-8") as file:
        for i in file:
            if(i.strip() == client_address[0]):
                addmsg(f"[{clock}] Banlanan kullanıcı {username}, {client_address[0]} üzerinden bağlanmaya çalıştı.")
                open("logs.txt", "a", encoding="utf-8").write(f"[{clock}] Banlanan kullanıcı {username}, {client_address[0]} üzerinden bağlanmaya çalıştı.\n")
                client_socket.close()
                return False
    if(client_address[0] in connected_address):
        addmsg(f"[{clock}] {connected_address[0]} ikinci kez bağlanmaya çalıştı.")
        open("logs.txt", "a", encoding="utf-8").write(f"[{clock}] {connected_address[0]} ikinci kez bağlanmaya çalıştı.\n")
        client_socket.close()
        return False
    return True
...
def command(event):
    ...
    if cmd == "exit": closing()
        ...
        addmsg(f"[{clock}] {ip} banlananlar listesine eklendi.")
        open("logs.txt", "a", encoding="utf-8").write(f"[{clock}] {ip} banlananlar listesine eklendi.\n")
    elif cmd.startswith("unban "):
        ip = cmd[6:].strip()
        with open("banned.txt", "r", encoding="utf-8") as file: lines = file.readlines()
        with open("banned.txt", "w", encoding="utf-8") as file:
            for line in lines:
                if line.strip() != ip:
                    file.write(line)
        addmsg(f"[{clock}] {ip} banlanan listesinden çıkartıldı.")
        open("logs.txt", "a", encoding="utf-8").write(f"[{clock}] {ip} banlanan listesinden çıkartıldı.\n")
    elif cmd.startswith("say "):
        message = cmd[4:].strip()
        broadcast(f"[{time.strftime('%H:%M')}] Sunucu: {message}")
    entry.delete(0, tk.END)

def start():
    server.listen()
    addmsg(f"[{clock}] Sunucu açıldı.")
    open("logs.txt", "a", encoding="utf-8").write(f"[{clock}] Sunucu açıldı.\n")
    while True:
...
except OSError:messagebox.showerror("Hata", "Sunucu zaten çalışıyor!"); closing()
