...
def username():
    ...
    def submit(event):
        ...
        if not name.isalpha(): messagebox.showerror("Warning", "Only alphabetic characters are allowed.")
        elif len(name) < 3: messagebox.showerror("Warning", "The name must be at least 3 characters long.")
        elif len(name) > 10: messagebox.showerror("Warning", "The name must be no more than 10 characters long.")
...
except Exception: messagebox.showerror("Error", "Server is offline!"); closing()
...
addmsg(f"[{clock}] {username} joined to chat.")
